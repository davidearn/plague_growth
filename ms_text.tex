...
%%
\ben{I *think* all I meant (no longer 100\% sure) is that, since we get reasonably consistent numbers across the different record types, that suggests that lag doesn't have a big effect. This is of course a weak argument, as we could imagine that we would get different growth rates from the different record types *if* they were not differently lagged, and the lag effects are canceling out the true differences.}
\david{changed ``are apparently'' to ``appear to be''}
